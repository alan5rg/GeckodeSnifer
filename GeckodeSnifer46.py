...
-----------------------------------------------------------
🦎 GECKOPENDIENTES (IDEAS EN CONSTANTE EXPLORACIÓN)
-----------------------------------------------------------
...
◇ Data & Variables GeckoSeeker
    Reporte Auditoria de Datos
...
geckoappversion = "4.6 Deluxe"

# ---------------------------------
# --- CORE GECKODESNIFER ENGINE --
# -------------------------------
class GeckodeSnifer:
    """
    Core GeckodeSnifer Engine
    Se encarga de generar el reporte completo
    """
    def __init__(self, project_path, single_file=None):
        ...
        # 🛡️ CAPA DE INVISIBILIDAD GECKÓNICA Ei2
        # Directorios que el sniffer ignorará por completo.
        self.ignored_dirs = {
            "__pycache__",
            ".git",
            "backups",
            "versions",      # El refugio de invisibilidad de Monkey Python Coding Circus & Team Cangurera
            "legacy",
            "basura"
        }
    ...
    def sniff_project(self):
        ...
        if self.single_file:
            self.files = 1
            self.sniff_file(self.single_file)
        else:
            for root, dirs, files in os.walk(self.project_path):
                # ✨ Aplicar invisibilidad Ei2: modificamos 'dirs' in-place
    ...
    def evaluate_structure(self):
        """
        Evalúa la estructura general del proyecto.
        Busca el punto de entrada real (if __name__ == '__main__':) 
        en lugar de depender del nombre del archivo.
        """
        self.report.append("\n📁 Evaluación de estructura\n")
        main_files = []
        # Buscamos en todos los archivos del proyecto (respetando la invisibilidad)
        for root, dirs, files in os.walk(self.project_path):
                ...
                if file.endswith(".py"):
                    ...
                    try:
                        with open(full_path, "r", encoding="utf-8") as f:
                            content = f.read()
                            # Detección del punto de entrada Geckónico
        ...
        if main_files:
            for main_file in main_files:
                self.report.append(f"✔ Punto de entrada detectado: {main_file} <--- ¡ESTE ES TU MAIN! 🚀\n")
        else:
            self.report.append("⚠ No se detectó un punto de entrada claro (if __name__ == '__main__':)\n")

    def get_node_line_count(self, node):
        """Calcula cuántas líneas ocupa un nodo de AST."""
        if hasattr(node, 'body'):
            # La diferencia entre el final y el inicio + 1
    ...
    def sniff_file(self, filepath):
        ...
        try:
            ...
            if module_doc:
                ...
                #self.report.append(f" '''{module_doc}'''\n")
            else:
                self.missing_docs += 1
                self.report.append(f"⚠ Módulo sin docstring\n")
            ...
            for node in tree.body:
                ...
                if isinstance(node, ast.ClassDef):
                    self.classes += 1
                    line_count = self.get_node_line_count(node)
                    self.report.append(f"\n🏛 Clase: {node.name} | Líneas: {line_count}")
                    
                    doc = ast.get_docstring(node)
                    if doc:
                        self.docstrings += 1
                        self.report.append(f"  └─ 📝 Docstring de clase presente:\n    '''{doc}'''\n")
                        #self.report.append("    '''" + doc + "'''\n")
                    else:
                        self.missing_docs += 1
                        self.report.append(f"  └─ ⚠ Clase sin docstring\n")

                    # --- Métodos dentro de la clase ---
                    for item in node.body:
                        if isinstance(item, ast.FunctionDef):
                            self.functions += 1
                            func_lines = self.get_node_line_count(item)
                            self.report.append(f"  └─ ⚙ Método: {item.name} | Líneas: {func_lines}")
                            
                            doc_m = ast.get_docstring(item)
                            if doc_m:
                                self.docstrings += 1
                                self.report.append(f"       └─ 📝 Docstring de método presente:\n    '''{doc_m}'''\n")
                                #self.report.append("    '''" + doc_m + "'''\n")
                            else:
                                self.missing_docs += 1
                                self.report.append(f"       └─ ⚠ Método sin docstring\n")
                
                # --- Funciones Globales ---
                elif isinstance(node, ast.FunctionDef):
                    self.functions += 1
                    line_count = self.get_node_line_count(node)
                    self.report.append(f"\n⚙ Función Global: {node.name} | Líneas: {line_count}")
                    
                    doc = ast.get_docstring(node)
                    if doc:
                        self.docstrings += 1
                        self.report.append(f"    └─ 📝 Docstring de función presente\n    '''{doc}'''\n")
                        #self.report.append("    '''" + doc + "'''\n")
                    else:
                        self.missing_docs += 1
                        self.report.append(f"    └─ ⚠ Función sin docstring\n")

        except Exception as e:
            self.report.append(f"❌ Error leyendo archivo: {e}")

    def sniff_data_and_vars(self, node, scope_type):
        """
        Estructura de Auditoría de Datos (Propuesta por Ei2)

        Si queremos que el reporte nos diga dónde vive la magia,
        propongo agregar este bloque de auditoría a tu sniff_file
        """
        # Detectar asignaciones (Variables)
        if isinstance(node, ast.Assign):
            for target in node.targets:
                if isinstance(target, ast.Name):
                    self.report.append(f"     └─ 📊 Variable: {target.id}")
                elif isinstance(target, ast.Attribute):
                    self.report.append(f"     └─ 📊 Var. Clase: {target.attr}")
                    
        # Detectar archivos de datos (Persistencia)
        if isinstance(node, ast.Constant) and isinstance(node.value, str):
            if any(ext in node.value for ext in ['.json', '.txt', '.db', '.sqlite', '.csv']):
                self.report.append(f"     └─ 💾 Persistencia detectada: {node.value}")
    ...
    def add_gecko_footprint(self):
        ...
        self.report.append(f"- Tool: GeckodeSnifer™ {geckoappversion}")
    ...
    def build_project_tree(self):
        ...
        def walk_level(directory, prefix=""):
            ...
            # 🛡️ Aplicar invisibilidad Ei2 a nivel árbol
    ...
    def build_inheritance_constellation(self):
        ...
        # Refactoreo por Ei2 para mayor impacto visual con Unicodes
        if not parents:
...
# ------------------------------
# --- MAIN GECKODESNIFER GUI --
# ----------------------------
class GeckodeSniferWindow(QMainWindow):
    """
    UX QMainWidows de GeckodeSnifer
    Levanta la GUI de interacción con el Teórico Ser Biológico del otro lado de la computadora
    """
    def __init__(self):
        super().__init__()
        self.setWindowTitle(f"🦎 GeckodeSnifer™ Geckeado por NOvaPythonisa v.{geckoappversion}")
    ...
    def init_ui(self):
        ...
        self.statusBar().showMessage(f" {geckoappversion} ")
    ...
    def wrap_tab(self, text_widget, default_name):
        ...
        copy_btn.clicked.connect(lambda: self.copy_report(text_widget))
    ...
    def copy_report(self, text_widget):
        content = text_widget.toPlainText()
        QApplication.clipboard().setText(content)
        self.status_label.setText("📋 Informe copiado al portapapeles ✔")
    ...
    def run_sniff(self):
        ...
        sniffer = GeckodeSnifer(project, single_file=single)
